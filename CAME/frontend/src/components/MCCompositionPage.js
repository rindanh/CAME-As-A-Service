...
class ChooseMCCard extends React.Component {

    constructor(props) {
        super(props)

        this.state = {
            el0: props.ch.detail[0],
            el1: props.ch.detail[1],
            at: props.ch.at,
            ch: props.ch,
            selected: props.ch.selected,
            modal: false,
            editable: false
        }

        this.handleChooseMCClick = this.handleChooseMCClick.bind(this)
        this.buttonExtendEditElement = this.buttonExtendEditElement.bind(this)
        this.buttonLeaveIt = this.buttonLeaveIt.bind(this)
    }
    ...
    handleChooseMCClick(mc, idx) {
        if (this.state.editable && idx===this.state.selected) {
            return
        }
    ...
    buttonExtendEditElement(ch) {
        let new_ch = this.state.ch
        new_ch.extend = true
        console.log("change the card to editable")
        this.setState({
            modal: false,
            editable: true,
            ch: new_ch
    ...
    elToPrint(el, a, idx) {
        if (a['idxFixedList'] !== undefined) {
            let conflicts = []
            if (idx===0) {
                for (let id of a.idxFixedList) {
                    conflicts.push(el[a['attribute']][id])
                }
                return {conflicts}
            } else {
                for (let id of a.idxConflictList) {
                    conflicts.push(el[a['attribute']][id])
                }
                return {conflicts}
            }
            ...
            if (a["attribute"] == "description") {
                return {"description": el[a['attribute']]}
            }
            return el
        }
    }

    editElement(edit, ch, a, idx) {
        console.log("di edit element", edit)
        console.log("ch", ch, a)

        let updated = edit.updated_src

        if (updated.conflicts) {
            let conflicts = updated.conflicts
            for (let conflict of conflicts) {
                for (let i in ch.detail[idx][a.attribute]) {
                    if (ch.detail[idx][a.attribute][i].name === conflict.name || ch.detail[idx][a.attribute][i].nameId === conflict.nameId) {
                        
                        console.log("conflict", conflict)
                        ch.detail[idx][a.attribute][i] = conflict
                        break
                    }
                }
            }
            console.log("ch after", ch)
        } else {
            if (updated.elementName) {
                ch.detail[idx].name = updated.elementName
            } else {
                ch.detail[idx] = updated
            }
            
        }
        console.log("ch afterafter", ch)
    }

    render() {

        let el0 = this.state.el0
        let el1 = this.state.el1
        ...
        if (name1 === undefined) {
            name0 = el0.characteristic
            name1 = el1.characteristic
        }

        return (
            <Row>
                <Col>
                    <Row className="d-block conflict-container">
                        <Card className={this.setClassName(0)} onClick={() => this.handleChooseMCClick(ch, 0)}>
                            <Card.Header as="h5">Name: {name0}</Card.Header>
                            <Card.Body>
                                {
                                    this.state.selected===0 && this.state.editable ? 
                                        <ReactJson 
                                            src={{"elementName": name0}} 
                                            onEdit={(edit) => this.editElement(edit, ch, at, 0)}
                                        />
                                ...
                                }
                                {
                                    at.map((a, idx) => (
                                        <div key={idx}>
                                            ...
                                            { this.state.selected===0 && this.state.editable ? 
                                                <ReactJson 
                                                    src={this.elToPrint(el0, a, 0)} 
                                                    onEdit={(edit) => this.editElement(edit, ch, a, 0)}
                                                />
                ...
                <Col>
                    <Row className="d-block conflict-container">
                        <Card className={this.setClassName(1)} onClick={() => this.handleChooseMCClick(ch, 1)}>
                            <Card.Header as="h5">Name: {name1}</Card.Header>
                            <Card.Body>
                                {
                                    this.state.selected===1 && this.state.editable ? 
                                        <ReactJson 
                                            src={{"elementName": name1}} 
                                            onEdit={(edit) => this.editElement(edit, ch, at, 1)}
                                        />
                                ...
                                }
                                {
                                    at.map((a, idx) => (
                                        <div key={idx}>
                                            ...
                                            { this.state.selected===1 && this.state.editable ? 
                                                <ReactJson 
                                                    src={this.elToPrint(el1, a, 1)} 
                                                    onEdit={(edit) => this.editElement(edit, ch, a, 1)}
                                                />
                ...
                <Modal show={this.state.modal} handleClose={e => this.modalClose(e)}>
                  <h4>Do you want to extend and edit the selected element or leave it?</h4>
                  <br/>
                  <Row className="d-flex justify-content-center">
                    <Col className="text-right">
                        <Button
                            onClick={() => this.buttonExtendEditElement(ch)}
                        >
                            Extend and Edit element
                        </Button>
                    </Col>
                    <Col className="text-left">
                        <Button
                            onClick={() => this.buttonLeaveIt(ch)}
                        >
                            Leave it
                        </Button>
                    </Col>
...
class MCCompositionPage extends React.Component {
    ...
    constructor(props) {
        ...
        this.buttonResolveConflicts = this.buttonResolveConflicts.bind(this)
    }
    ...
    async composeMC() {
        ...
        var chunksToBeComposed = this.props.project.method_chunks.slice()
        this.delId(chunksToBeComposed)
        var numberOfChunkToBeMerge = chunksToBeComposed.length

        if (numberOfChunkToBeMerge >= 2) {
         

            let promiseInit = new Promise((resolve, reject) => {
                let initChunk = chunksToBeComposed.shift()
                this.setState({
                    chunksToBeComposed: chunksToBeComposed
                })
                resolve(initChunk)
            })

            promiseInit.then((initChunk) => {
                this.compose2chunks(initChunk)
            })
    ...
    async compose2chunks(initChunk) {
        let chunksToBeComposed = this.state.chunksToBeComposed.slice()

        let secondChunk = chunksToBeComposed.shift()
        ...
        let currentChunksToBeMerged = [initChunk, secondChunk]
        console.log("currentChunksToBeMerged", currentChunksToBeMerged)

        var promiseBeforeCompose = new Promise((resolve, reject) => {

            this.setState({
                chunksToBeMerged: currentChunksToBeMerged,
                chunksToBeComposed: chunksToBeComposed
            })

            for (let task of currentChunksToBeMerged) {
        ...
        promiseBeforeCompose.then(() => {
            ...
            promise1.then(() => {
                if (this.isConflictsSolved()) {
                    let mergeAllPromise = new Promise((resolve, reject) => {
                        this.mergeAll()
                        resolve()
                    })

                    mergeAllPromise.then(() => {
                        if (this.state.chunksToBeComposed.length > 0) {
                            let init = this.state.createdChunk
                            this.compose2chunks(init)
                        } else {
                    ...
                    alert("There are some conflicts. Please scroll to resolve")
                    this.setState({
                        showConflicts: true
                    })
                }
    ...
    handleConflicts() {
        ...
        var promise2 = new Promise((resolve, reject) => {
            ...
            for (let as of this.state.activitySpacesConflicts) {
                console.log("as", as)
                ...
                for (let at of ats) {
                    if (at.idxFixedList !== undefined) {
                        for (let i=0; i<at.idxFixedList.length; i++) {
                            activitySpaces[as.idx][at.attribute][at.idxFixedList[i]] = as.detail[id][at.attribute][at.idxConflictList[i]]
                        }
                        
                    } else {

                        activitySpaces[as.idx][at.attribute] = as.detail[id][at.attribute]
                    }
                    
                }

            }
            for (let comp of this.state.competenciesConflicts) {
                console.log("comp", comp)
                let id = comp.selected
                let ats = comp.at

                for (let at of ats) {
                    if (at.idxFixedList !== undefined) {
                        for (let i=0; i<at.idxFixedList.length; i++) {
                            competencies[comp.idx][at.attribute][at.idxFixedList[i]] = comp.detail[id][at.attribute][at.idxConflictList[i]]
                        }
                        
                    } else {
                        competencies[comp.idx][at.attribute] = comp.detail[id][at.attribute]
                    }
                    
                }
            }
            for (let a of this.state.alphasConflicts) {
                console.log("a", a)
                let id = a.selected || 0
                let ats = a.at
                ...
                // for (let at of ats) {
                    // if (at.idxFixedList) {
                        ...
                        if (a.extend) {
                            if (a.detail[0].name === a.detail[1].name) {
                                a.detail[id].name = a.detail[id].name.concat(" Extended")
                            }

                            alphas.push(a.detail[1])

                            
                            
                        } else {
                            alphas[a.idx] = a.detail[id]
                        }

                        // for (let i=0; i<at.idxFixedList.length; i++) {
                        //     alphas[a.idx][at.attribute][at.idxFixedList[i]] = a.detail[id][at.attribute][at.idxConflictList[i]]
                        // }
                        
                    // } else {
                    //     // alphas[a.idx][at.attribute] = a.detail[id][at.attribute]
                    // }
                    
                // }

            }
            for (let p of this.state.patternsConflicts) {
                console.log("p", p)
                ...
                for (let at of ats) {
                    if (at.idxFixedList) {
                        for (let i=0; i<at.idxFixedList.length; i++) {
                            patterns[p.idx][at.attribute][at.idxFixedList[i]] = p.detail[id][at.attribute][at.idxConflictList[i]]
                        }
                        
                    } else {
                        patterns[p.idx][at.attribute] = p.detail[id][at.attribute]
                    }
                    
                }
            }
            for (let char of this.state.characteristicsConflicts) {
                console.log("char", char)

                let id = char.selected || 0
                let ats = char.at

                // for (let at of ats) {
                //     if (at.idxFixedList) {
                //         for (let i=0; i<at.idxFixedList.length; i++) {
                //             characteristics[char.idx][at.attribute][at.idxFixedList[i]] = char.detail[id][at.attribute][at.idxConflictList[i]]
                //         }
                        
                //     } else {
                //         characteristics[char.idx][at.attribute] = char.detail[id][at.attribute]
                //     }
                    
                // }

                // if (char.extend) {
                //     if (characteristics[char.idx].characteristic === char.detail[id].characteristic) {
                //         char.detail[id].characteristic = char.detail[id].characteristic.concat("Extended")
                //     }
                //     characteristics.push(char.detail[id])
                    
                // } else {
                //     characteristics[char.idx] = char.detail[id]
                // }
            ...
            let name0 = this.state.chunksToBeMerged[0].name.concat(" + ")
            let name1 = this.state.chunksToBeMerged[1].name

            let id0 = this.state.chunksToBeMerged[0].nameId
            let id1 = this.state.chunksToBeMerged[1].nameId
            ...
            this.setState({
                createdChunk: {
                    ...
                    patterns: patterns,
                    name: name0.concat(name1),
                    nameId: id0.concat(id1)
                }
            })
            resolve()
        })
              ...
              for (var j = 0; j < charTemp.length; j++) {
                ...
                if (firstCurrentChar.characteristic === secondCurrentChar.characteristic) {
                    ...
                    if (firstCurrentChar.value !== secondCurrentChar.value) {
                        ...
                        let newObj = this.conflictResolution(secondCurrentChar, firstCurrentChar, j, [{"attribute":"value"}])
              ...
              for (var j = 0; j < aSTemp.length; j++) {
                ...
                if (firstCurrentAS.name === secondCurrentAS.name || firstCurrentAS.nameId === secondCurrentAS.nameId) {
                    ...
                    if (conflictAt.length > 0) {
                        ...
                        let newRes = this.conflictResolution(secondCurrentAS, firstCurrentAS, j, conflictAt)
            ...
            for (var i = 0; i < secondChunkCompetencies.length; i++) {
              ...
              for (var j = 0; j < competenciesTemp.length; j++) {
                ...
                if (firstCurrentCompetency.name === secondCurrentCompetency.name || firstCurrentCompetency.nameId === secondCurrentCompetency.nameId) {
                    ...
                    if (conflictAt.length > 0) {
                        // semantik ga sama, jadinya harus bikin object merge resolution
                        let newRes = this.conflictResolution(secondCurrentCompetency, firstCurrentCompetency, j, conflictAt)
                        conflicts.push(newRes)
                                    ...
                                    if (tempA[i].name === tempB[j].name || tempA[i].nameId === tempB[j].nameId) {
                                        ...
                                        if (JSON.stringify(tempA[i]) !== JSON.stringify(tempB[j])) {
                                            ...
                                            for (let a of arr) {
                                                if (a.attribute === k) {
                                                    exist = true
                                                    a.idxFixedList.push(j)
                                                    a.idxConflictList.push(i)
                                                    break
                                                }
                                            }
    ...
    conflictResolution(first, second, idxOnConflict, conflictAt) {
            ...
            for (var i = 0; i < secondChunkAlphas.length; i++) {
              ...
              for (var j = 0; j < alphasTemp.length; j++) {
                ...
                if (firstCurrentAlpha.name === secondCurrentAlpha.name || firstCurrentAlpha.nameId === secondCurrentAlpha.nameId) {
                    ...
                    if (conflictAt.length > 0) {
                        // semantik ga sama, jadinya harus bikin object merge resolution
                        let newRes = this.conflictResolution(secondCurrentAlpha, firstCurrentAlpha, j, conflictAt)
                        conflicts.push(newRes)
            ...
            for (var i = 0; i < secondChunkPatterns.length; i++) {
              ...
              for (var j = 0; j < patternsTemp.length; j++) {
                ...
                if (firstCurrentPatterns.name === secondCurrentPatterns.name || firstCurrentPatterns.nameId === secondCurrentPatterns.nameId) {
                    ...
                    if (conflictAt.length > 0) {
                        // semantik ga sama, jadinya harus bikin object merge resolution
                        let newRes = this.conflictResolution(secondCurrentPatterns, firstCurrentPatterns, j, conflictAt)
                        conflicts.push(newRes)
    ...
    saveCreatedMethod() {
        ...
        let createdChunk = this.state.createdChunk
        delete createdChunk['name']
        delete createdChunk['nameId']
    ...
    buttonResolveConflicts(conflicts) {
        console.log("button pressed", conflicts)
    ...
    render() {
        ...
        const chunk1 = this.state.chunksToBeMerged[0]
        const chunk2 = this.state.chunksToBeMerged[1]

        const zero = 0
        const one = 1

        let conflictsChar = this.state.characteristicsConflicts.slice()
        let conflictsAS = this.state.activitySpacesConflicts.slice()
        let conflictsAlphas = this.state.alphasConflicts.slice()
        let conflictsCompetencies = this.state.competenciesConflicts.slice()
        let conflictsPatterns = this.state.patternsConflicts.slice()

        let conflicts = [conflictsChar, conflictsAS, conflictsAlphas, conflictsCompetencies, conflictsPatterns]
                                    ...
                                    <Form>
                                        <Form.Group controlId="name">
                                            ...
                                            <Form.Control
                                              ...
                                              value={this.state.name || this.state.createdChunk.name}
                        ...
                        { this.state.showConflicts ?
                            <div>
                                <h2>
                                    ...
                                    between MC A and MC B
                                </h2>
                                <br/>
                                <h4>
                                    Please choose one of the two or do extension for one of them
                                </h4>

                                <br/>
                                <Row>
                                    <Col className="text-center">
                                        <h5>Chunk:<br/>{chunk1.name}</h5>
                                    </Col>
                                    <Col className="text-center">
                                        <h5>Chunk:<br/>{chunk2.name}</h5>
                                    </Col>
                                </Row>

                                { conflictsChar.length > 0 ?
                                    <div>
                                        <br/>
                                        <Row className="d-flex justify-content-center">
                                ...
                                { conflictsCompetencies.length > 0 ?
                                    <div>
                                        <br/>
                                        <Row className="d-flex justify-content-center">
                                            <h4>Competencies</h4>
                                        ...
                                        {
                                            conflictsCompetencies.map((ch, idx) => (
                                                <div>
                                                    <ChooseMCCard
                                                        ch={ch}
                                                    />
                                ...
                                <Row className="d-flex justify-content-center">
                                    <Button
                                        variant="success"
                                        className="float-right"
                                        onClick={() => this.buttonResolveConflicts(conflicts)}
                                    >
                                        Resolve conflicts
                                    </Button>
                                </Row>
